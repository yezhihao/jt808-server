...
public class JTClientEndpoint {
    ...
    private T0001 buildResult(Header requestHeader, int resultCode) {
        T0001 result = new T0001();
        Header header = new Header().copyBy(requestHeader);
        header.setMessageId(JT808.终端通用应答);
        header.setMobileNo(mobileNo);
        header.setSerialNo(serialNo.addAndGet(1));
